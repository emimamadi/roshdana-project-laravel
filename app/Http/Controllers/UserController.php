    ...
    public function dashboard()
    {
        if (Auth::check()) {
            ...
            $users_req = User::whereHas('follow', function (Builder $query) {
                $query->where('following_id', '=', Auth::user()->id)->Where("status", "=", "pending");
            })->get();


            $users_acc = User::whereHas('follow', function (Builder $query) {
                $query->where('following_id', '=', Auth::user()->id)->Where("status", "=", "accepted");
            })->get();


            $other_users = User::whereHas('follow', function (Builder $query) {
            ...
            return view('dashboard', compact(['users', 'users_req', 'users_acc', 'other_users']));
        } else {
